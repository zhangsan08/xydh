<template>
  <div class="linksort">
        <p>肝不动了。界面丑了点。仅支持电脑端操作。凑合用吧。书签排序属于批量写入，即使你只改变某一个书签的位置，带来的也是全部书签的改写操作。大家尽量一次性改好顺序。不要一直改了。真的没钱再给服务器升配了。有能力的请用金钱羞辱我，谢谢！</p>
        <el-collapse accordion v-model="FolderID" @change="getLinksin(FolderID)" v-loading="loading">
            ...
            <el-collapse-item :name="Folder.id">
				...
				<!-- 这是文件夹里的内容 -->
				<div>
                    拖动书签进行排序 <el-button type="text" @click="sortLink()" >点击保存</el-button>
                    <el-tree
                        :props="defaultProps"
                        :data="links"
                        node-key="id"
